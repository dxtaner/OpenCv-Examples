...
ret, frame = cap.read()
if ret == False:
    print("Warning: Unable to access the camera!")

face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
face_rect = face_cascade.detectMultiScale(frame)

(face_x, face_y, w, h) = tuple(face_rect[0])
track_window = (face_x, face_y, w, h)  # Initial tracking window for MeanShift

roi = frame[face_y:face_y + h, face_x:face_x + w]  # Crop the face region
