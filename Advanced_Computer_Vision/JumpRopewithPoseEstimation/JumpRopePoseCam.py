...
def findAngle(img, p1, p2, p3, lmList, draw=True):
    ...
    if draw:
        cv2.line(img, (x1, y1), (x2, y2), (0, 128, 255), 3)  # Turuncu renk
        cv2.line(img, (x2, y2), (x3, y3), (0, 128, 255), 3)  # Turuncu renk
        cv2.circle(img, (x1, y1), 10, (255, 255, 0), cv2.FILLED)  # Açık mavi renk
        cv2.circle(img, (x2, y2), 10, (255, 255, 0), cv2.FILLED)  # Açık mavi renk
        cv2.circle(img, (x3, y3), 10, (255, 255, 0), cv2.FILLED)  # Açık mavi renk

        cv2.circle(img, (x1, y1), 15, (255, 255, 0))
        cv2.circle(img, (x2, y2), 15, (255, 255, 0))
        cv2.circle(img, (x3, y3), 15, (255, 255, 0))

        cv2.putText(img, str(int(angle)), (x2 - 40, y2 + 40), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 255), 2)  # Sarı renk
    return angle
...
# Kameradan video akışını aç
cap = cv2.VideoCapture(0)  # Eğer bir video dosyası kullanılıyorsa, dosya adını belirtin

# FPS hesaplamak için zaman değişkenleri
pTime = 0
cTime = 0
